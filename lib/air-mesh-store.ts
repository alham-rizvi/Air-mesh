...
export type AccentColor = '#F05A28' | '#10A37F' | '#2F80ED' | '#8E5CF6' | '#D97706';
...
export const useThemeStore = create<{ mode: ThemeMode; accent: AccentColor; setMode: (mode: ThemeMode) => void; setAccent: (accent: AccentColor) => void }>(((set) => ({ mode: 'dark', accent: '#F05A28', setMode: (mode) => set({ mode }), setAccent: (accent) => set({ accent }) })));
...
export const useChatStore = create<{ chats: Chat[]; messages: Record<string, Message[]>; addMessage: (chatId: string, text: string, messageId?: string) => string; updateDelivery: (chatId: string, messageId: string, delivery: Delivery) => void; addDirect: (contact: Contact) => string; addGroup: (name: string, memberIds: string[]) => string; reset: () => void }>(((set, get) => ({
  chats: mockChats,
  messages: {},
  addMessage: (chatId, text, messageId) => { const id = messageId ?? String(Date.now()); set((state) => ({ messages: { ...state.messages, [chatId]: [...(state.messages[chatId] || []), { id, text, time: 'Now', sent: true, delivery: 'queued', relay: 'Local encrypted outbox' }] } })); return id; },
