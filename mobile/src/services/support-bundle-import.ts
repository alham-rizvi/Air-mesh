...
export function parseRedactedSupportBundle(raw: string): ImportedSupportBundleSummary {
  ...
  array(topology.edges, 'observed_topology.edges');
  ...
  return { generatedAt: string(root.generated_at, 'generated_at'), transport: string(status.transport, 'mesh_status.transport'), connectedPeers: nodes.length, routes: routes.length, senderQueue: outbox.length, relayQueue: relay.length, retryEntries: retry.length };
}
